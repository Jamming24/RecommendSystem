...
# @File    : seq2seq_with_attention.py
# @Software: PyCharm
...
class Seq2SeqModel(object):
    ...
    def get_batch(self, data, bucket_id):
        ...
        for _ in xrange(self.batch_size):
            ...
            # pad和反转Encoder 的输入数据..
            encoder_pad = [tensorflow_Machine_Translation_DataProcess.PAD_ID] * (encoder_size - len(encoder_input))
            encoder_inputs.append(list(reversed(encoder_input + encoder_pad)))

            # 为Decoder输入数据添加一个额外的"GO", 并且进行pad.
            decoder_pad_size = decoder_size - len(decoder_input) - 1
            decoder_inputs.append([tensorflow_Machine_Translation_DataProcess.GO_ID] + decoder_input +
                                  [tensorflow_Machine_Translation_DataProcess.PAD_ID] * decoder_pad_size)
        ...
        for length_idx in xrange(decoder_size):
            ...
            for batch_idx in xrange(self.batch_size):
                ...
                if length_idx == decoder_size - 1 or target == tensorflow_Machine_Translation_DataProcess.PAD_ID:
