    ...
    def __init__(self, config, w2i_target, useTeacherForcing=True, useAttention=True, useBeamSearch=1):
        ...
        with tf.variable_scope("decoder"):
            ...
            with tf.variable_scope("gru_cell"):
                ...
                if useAttention:
                    if useBeamSearch > 1:
                        tiled_encoder_outputs = tile_batch(encoder_outputs, multiplier=useBeamSearch)
                        tiled_sequence_length = tile_batch(self.seq_inputs_length,
                                                                              multiplier=useBeamSearch)
                        attention_mechanism = BahdanauAttention(num_units=config.hidden_dim,
                                                                                   memory=tiled_encoder_outputs,
                                                                                   memory_sequence_length=tiled_sequence_length)
                        decoder_cell = AttentionWrapper(decoder_cell, attention_mechanism)
                        tiled_encoder_final_state = tile_batch(encoder_state,
                                                                                  multiplier=useBeamSearch)
                        ...
                        attention_mechanism = BahdanauAttention(num_units=config.hidden_dim, memory=encoder_outputs,
                    ...
                    if useBeamSearch > 1:
                        decoder_initial_state = tile_batch(encoder_state, multiplier=useBeamSearch)
                    else:
                        decoder_initial_state = encoder_state

            if useBeamSearch > 1:
                decoder = BeamSearchDecoder(decoder_cell, decoder_embedding, tokens_go, w2i_target["_EOS"], decoder_initial_state, beam_width=useBeamSearch, output_layer=tf.layers.Dense(config.target_vocab_size))
            else:
                decoder = BasicDecoder(decoder_cell, helper, decoder_initial_state, output_layer=tf.layers.Dense(config.target_vocab_size))

            decoder_outputs, decoder_state, final_sequence_lengths = dynamic_decode(decoder, maximum_iterations=tf.reduce_max(self.seq_targets_length))
        ...
        if useBeamSearch > 1:
            ...
            self.loss = sequence_loss(logits=decoder_logits, targets=self.seq_targets,
