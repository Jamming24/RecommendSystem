...
class Reward(object):
    def __init__(self, lstm, update_rate):
        self.lstm = lstm
        self.update_rate = update_rate  # 更新比率

        self.num_vocabulary = self.lstm.num_vocabulary  # 词库数量
        self.batch_size = self.lstm.batch_size  # lstm 批大小
        self.emb_dim = self.lstm.emb_dim  # 词嵌入维度
        self.hidden_dim = self.lstm.hidden_dim  # 隐藏节点数
        self.sequence_length = self.lstm.sequence_length  # 序列长度
        self.start_token = tf.identity(self.lstm.start_token)  # 起始令牌
        self.learning_rate = self.lstm.learning_rate  #学习率
    ...
    def get_reward(self, sess, input_x, rollout_num, discriminator):
        # 用于reward的样例生成组合——改
        def combin_orc_gen_reward(gen):
            result = []
            with open('save/oracle.txt', 'r', encoding='utf-8') as orar:
                for x, y in zip(list(orar), list(gen)):
                    result.append(x.strip().split() + list(y))
            return result
        rewards = []
        for i in range(rollout_num):
            for given_num in range(1, len(input_x[0])):
                ...
                result = combin_orc_gen_reward(samples)
                # 替换原始的单句样例，变成句对样例——改
                # feed = {discriminator.input_x: samples}
                feed = {discriminator.input_x: result}
            ...
            # 替换原始的单句样例，变成句对样例——改
            result2 = combin_orc_gen_reward(input_x)
            # feed = {discriminator.input_x: input_x}
            feed = {discriminator.input_x: result2}
            ypred_for_auc = sess.run(discriminator.ypred_for_auc, feed)
            ypred = np.array([item[1] for item in ypred_for_auc])
            if i == 0:
                rewards.append(ypred)
                ...
                # ——改
                # rewards[(len(result2[0])-1)] += ypred
        # transpose 没有任何额外的参数，代表执行转置操作
    ...
    # 调用更新的参数方法，g_embeddings，g_recurrent_unit，g_output_unit
